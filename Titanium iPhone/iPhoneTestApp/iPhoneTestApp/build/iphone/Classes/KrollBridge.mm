...
@implementation iPhoneTestApp3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_iphonetestapp3);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any iPhoneTestApp3 patches in JS before a context is loaded... nice for 
...
{
	// create iPhoneTestApp3 global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_iphonetestapp3 = [[iPhoneTestApp3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_iphonetestapp3];
	
	NSString *_iphonetestapp3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_iphonetestapp3NS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_iphonetestapp3 valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.iphonetestapp3.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
